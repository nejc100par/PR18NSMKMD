...
f = codecs.open("pnose98.txt", encoding='cp852')
i = open("pnose1998.csv","w")
...
for vrsta in f:
    ...
    if 1<len(vrsta):
        ...
        Povzrocitelj = kaj[vrsta[9].strip()] if vrsta[9].strip()!="" else ""
        Starost = vrsta[10:12].strip() if vrsta[10:12].strip()!="" else ""
        Spol = spol[vrsta[14].strip()] if vrsta[14].strip()!="" else ""
        UEStalnegaPrebivalisca = loob[int(vrsta[15:19].strip())] if vrsta[15:19].strip() !="" else ""
        Drzavljanstvo = lodz[int(vrsta[19:22].strip())] if vrsta[19:22].strip()!="" and int(vrsta[19:22]) in lodz  else ""

        PoskodbaUdelezenca= ""
        VrstaUdelezenca= ""
        UporabaVarnostnegaPasu= ""
        VozniskiStazVLetih= "0"
        VozniskiStazVMesecih = "0"

        #print(len(vrsta))
        if len(vrsta)>22:
            PoskodbaUdelezenca = prpo[vrsta[22].strip()] if vrsta[22].strip()!="" else ""
        if len(vrsta) > 23:
            VrstaUdelezenca = prvu[vrsta[23:25].strip()] if vrsta[23:25].strip()!="" else ""
        if len(vrsta) > 25:
            UporabaVarnostnegaPasu = pas[vrsta[25].strip()] if vrsta[25].strip()!="" else ""
        if len(vrsta) > 26:
            VozniskiStazVLetih = vrsta[26:28].strip() if vrsta[26:28].strip()!="" else "0"
        if len(vrsta) > 28:
            VozniskiStazVMesecih = vrsta[28:30].strip() if  vrsta[28:30].strip()!="" else "0"

        VrednostStrokovnegaPregleda = "0"
        VrednostAlkotesta = "0"

        if p:
            VrednostAlkotesta = vrsta[30:34].strip() if vrsta[30:34].strip()!="" else "0"
            VrednostStrokovnegaPregleda = vrsta[34:38].strip() if vrsta[34:38].strip()!="" else "0"
